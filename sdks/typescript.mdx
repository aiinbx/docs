...
export const POST = createNextRouteHandler({
  onInboundEmail: async ({ payload }) => {
    ...
    const { object } = await generateObject({
      ...
      schema: { responseHtml: 'string' },
      system: 'You are a helpful support agent for AI Inbx.',
      prompt: `Thread so far: ${threadToLLMString(thread)}\nNew email: ${emailToLLMString(email)}`,
    });
